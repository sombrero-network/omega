...
/**
 * Generic converter class used to translate the market information contained in
 * a BucketObject and/or Price instances.
 *
...
public class Converter {
    ...
    /**
     * Constructor meant to be used trying to perform a conversion and in
     * possession of a Price object.
     */
    public Converter() {
    }
    
    /**
     * Constructor meant to be used when trying to perform a conversion and in
     * possession of a BucketObject, typically resulting from a
     * 'get_market_history' API call.
     * 
     * @param base
     * @param quote
     * @param bucket
    ...
    /**
     * Method used to obtain the equivalence between two assets considering
     * their precisions and given the specific time bucket passed in the
     * constructor.
     *
     * The resulting double value will tell us how much of a given asset, a unit
     * of its pair is worth.
     *
     * The second argument is used to specify which of the assets should be
     * taken as a unit reference.
     *
     * For instance if used with the BASE_TO_QUOTE constant, this method will
     * tell us how many of the quote asset will make up for a unit of the base
     * asset. And the opposite is true for the QUOTE_TO_BASE contant.
     *
     * @param bucketAttribute:
     *            The desired bucket attribute to take in consideration. Can be
     *            any of the following: OPEN_VALUE, CLOSE_VALUE, HIGH_VALUE or
     *            LOW_VALUE.
     * @param direction:
     *            One of two constants 'BASE_TO_QUOTE' or 'QUOTE_TO_BASE' used
     *            to specify which of the two assets is the one used as a
     *            unitary reference.
     * @return: double value representing how much of one asset, a unit of the
     *          paired asset was worth at the point in time specified by the
     *          time bucket and the bucket parameter.
     */
    public double getConversionRate(int bucketAttribute, int direction) {
        ...
        double basePrecisionAdjusted = baseValue.divide(BigDecimal.valueOf((long) Math.pow(10, base.getPrecision())))
                .doubleValue();
        double quotePrecisionAdjusted = quoteValue.divide(BigDecimal.valueOf((long) Math.pow(10, quote.getPrecision())))
                .doubleValue();
    ...
    /**
     * Converts a given asset amount to the corresponding pair used when
     * creating this class.
     * 
     * @param assetAmount:
     *            The asset to convert from.
     * @param bucketAttribute:
     *            The bucket attribute to use as a reference. Possible values
     *            are OPEN_VALUE, CLOSE_VALUE, HIGH_VALUE or LOW_VALUE.
     * @return: The converted value in base units, that is the number of a unit
     *          x 10^precision
     */
    ...
    /**
     * Method used to obtain the conversion rate between two assets given in a
     * Price instance as recovered by the 'get_limit_orders' API call.
     *
     * The same rules that apply for the
     * {@link #getConversionRate(int bucketAttribute, int direction)
     * getConversionRate} are valid for the 'direction' argument.
     *
     * @param price:
     *            The Price object instance
     * @param direction:
     *            The direction from which to perform the conversion, can be
     *            only one of BASE_TO_QUOTE or QUOTE_TO_BASE.
    ...
    public double getConversionRate(Price price, int direction) {
        ...
        // baseValue.longValue(), quoteValue.longValue()));
        ...
        // System.out.println(String.format("conversion rate: %.4f, precision
        // factor: %.2f", conversionRate, precisionFactor));
