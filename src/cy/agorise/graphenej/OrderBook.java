 ...
 *
 * It also provides a handy method that should return the appropriate
 * LimitOrderCreateOperation object needed in case the user wants to perform
 * market-priced operations.
 *
 * It is important to keep the order book updated, ideally by listening to
 * blockchain events, and calling the 'update' method.
 *
 */
public class OrderBook {
    ...
    public void update(LimitOrder limitOrder) {
        // TODO: Implement the method that will update a single limit order from
        // the order book
    }
    
    /**
     * High level method used to exchange a specific amount of an asset (The
     * base) for another one (The quote) at market value.
     *
     * It should analyze the order book and figure out the optimal amount of the
     * base asset to give away in order to obtain the desired amount of the
     * quote asset.
     *
     * @param seller:
     *            User account of the seller, used to build the limit order
     *            create operation
     * @param myBaseAsset:
     *            The asset the user is willing to give away
     * @param myQuoteAmount:
     *            The amount of a given asset the user wants
     * @param expiration:
     *            The expiration time of the limit order
     *
     * @return An instance of the LimitOrderCreateOperation class, which is
    ...
    public LimitOrderCreateOperation exchange(UserAccount seller, AssetAmount baseAmount, Asset quoteAsset,
            int expiration) {
    ...
    /**
     * Method that calculates the amount of an asset that we will obtain (the
     * quote amount) if we trade a known fixed amount of the asset we already
     * have (the base amount).
     *
     * @param baseAmount
     *            The fixed amount of the asset we have and want to sell
    ...
    public UnsignedLong calculateObtainedQuote(AssetAmount baseAmount) {
        ...
        for (int i = 0; i < limitOrders.size() && myBase.compareTo(UnsignedLong.ZERO) > 0; i++) {
            ...
            if (order.getSellPrice().quote.getAsset().equals(baseAmount.getAsset())) {
                ...
                UnsignedLong myQuote = UnsignedLong
                        .valueOf((long) (myBase.times(orderBase).doubleValue() / (orderQuote.doubleValue())));
                if (myQuote.compareTo(availableBase) > 0) {
                    ...
                    // We need more than this order can offer us, but have to
                    // take in consideration how much there really is.
                    // (order base / order quote) x (available order base /
                    // order base)
                    UnsignedLong thisBatch = UnsignedLong.valueOf((long) (orderBase.times(availableBase).doubleValue()
                            / orderQuote.times(orderBase).doubleValue()));
                    obtainedQuote = obtainedQuote.plus(thisBatch);
                } else {
                    // This order consumes all our base asset
                    // obtained_quote = obtained_quote + (my base * order_base /
                    // order_quote)
    ...
    /**
     * Method that calculates the amount of an asset that we will consume (the
     * base amount) if we want to obtain a known fixed amount of another asset
     * (the quote amount).
     * 
     * @param quoteAmount
     *            The fixed amount of an asset that we want to obtain
     * @return The amount of an asset we already have that will be consumed by
     *         the trade
     */
        ...
        for (int i = 0; i < limitOrders.size() && myQuote.compareTo(UnsignedLong.ZERO) > 0; i++) {
            ...
            if (order.getSellPrice().base.getAsset().equals(quoteAmount.getAsset())) {
                ...
                if (forSale.compareTo(myQuote) > 0) {
                    ...
                    obtainedBase = obtainedBase.plus(UnsignedLong.valueOf(
                            (long) (myQuote.doubleValue() * orderQuote.doubleValue() / orderBase.doubleValue())));
                    myQuote = UnsignedLong.ZERO;
                } else {
                    // Found an order that partially fills our needs
                    obtainedBase = obtainedBase.plus(UnsignedLong.valueOf(
                            (long) (forSale.doubleValue() * orderQuote.doubleValue() / orderBase.doubleValue())));
