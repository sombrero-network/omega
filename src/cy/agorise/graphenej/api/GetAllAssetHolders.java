 ...
 *
 * @see <a href="https://goo.gl/AgTSLU">get_all_asset_holders API doc (source
 *      code ref.)</a>
 */
public class GetAllAssetHolders extends BaseGrapheneHandler {
     ...
     *            A class implementing the WitnessResponseListener interface.
     *            This should be implemented by the party interested in being
     *            notified about the success/failure of the transaction
     *            broadcast operation.
     */
    ...
    public void onTextFrame(WebSocket websocket, WebSocketFrame frame) throws Exception {
        ...
        if (baseResponse.error != null) {
            ...
            if (baseResponse.id == LOGIN_ID) {
                ...
                ApiCall apiCall = new ApiCall(assetApiId, RPC.CALL_GET_ALL_ASSET_HOLDERS, emptyParams, RPC.VERSION,
                        currentId);
                websocket.sendText(apiCall.toJsonString());
            } else if (baseResponse.id == GET_ALL_ASSET_HOLDERS_COUNT) {
                Type AssetTokenHolders = new TypeToken<WitnessResponse<List<AssetHolderCount>>>() {
                }.getType();
                GsonBuilder builder = new GsonBuilder();
                builder.registerTypeAdapter(AssetHolderCount.class, new AssetHolderCount.HoldersCountDeserializer());
                WitnessResponse<List<AssetHolderCount>> witnessResponse = builder.create().fromJson(response,
                        AssetTokenHolders);
