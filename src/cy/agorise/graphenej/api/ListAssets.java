...
/**
 * WebSocketAdapter class used to send a request a 'list_assets' API call to the
 * witness node.
 *
 * The API imposes a limit of of 100 assets per request, but if the user of this
 * class wants to get a list of all assets, the LIST_ALL constant must be used
 * as second argument in the constructor. Internally we are going to perform
 * multiple calls in order to satisfy the user's request.
 *
 * @see: <a href=
 *       "http://docs.bitshares.org/development/namespaces/app.html"></a>
 */
public class ListAssets extends BaseGrapheneHandler {
    /**
     * Constant that must be used as argument to the constructor of this class
     * to indicate that the user wants to get all existing assets.
     */
    public static final int LIST_ALL = -1;
    
    /**
     * Internal constant used to represent the maximum limit of assets retrieved
     * in one call.
     */
     ...
     * Constructor
     *
     * @param lowerBoundSymbol
     *            Lower bound of symbol names to retrieve
     * @param limit
     *            Maximum number of assets to fetch, if the constant LIST_ALL is
     *            passed, all existing assets will be retrieved.
     * @param oneTime
    ...
    public void onTextFrame(WebSocket websocket, WebSocketFrame frame) throws Exception {
        ...
        Type LookupAssetSymbolsResponse = new TypeToken<WitnessResponse<List<Asset>>>() {
        }.getType();
        gsonBuilder.registerTypeAdapter(Asset.class, new Asset.AssetDeserializer());
        WitnessResponse<List<Asset>> witnessResponse = gsonBuilder.create().fromJson(response,
                LookupAssetSymbolsResponse);
        if (this.limit != LIST_ALL && this.limit < MAX_BATCH_SIZE) {
            ...
            // Updating counter to keep track of how many batches we already
            // retrieved.
            requestCounter++;
            if (this.assets == null) {
            ...
            if (this.limit == LIST_ALL && witnessResponse.result.size() < MAX_BATCH_SIZE) {
                // In case we requested all assets, we might be in the last
                // round whenever
                ...
                int nextBatch = this.limit == LIST_ALL ? MAX_BATCH_SIZE
                        : this.limit - (MAX_BATCH_SIZE * requestCounter);
                ArrayList<Serializable> params = new ArrayList<>();
                params.add(this.lowerBound);
