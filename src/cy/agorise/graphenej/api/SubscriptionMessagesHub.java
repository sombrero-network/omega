...
/**
 * A WebSocket adapter prepared to be used as a basic dispatch hub for
...
public class SubscriptionMessagesHub extends BaseGrapheneHandler implements SubscriptionHub {
    ...
    /**
     * Id attributed to the indivitual 'get_objects' API call required for a
     * fine-grained subscription request.
     */
     ...
     * set_subscribe_callback API with the clear_filter parameter set to false,
     * meaning that it will only receive automatic updates from objects we
     * register.
     *
     * A list of ObjectTypes must be provided, otherwise we won't get any
     * update.
     *
     * @param user
     *            User name, in case the node to which we're going to connect to
     *            requires authentication
     * @param password
     *            Password, same as above
     * @param clearFilter
     *            Whether to automatically subscribe of not to the notification
     *            feed.
     * @param errorListener
     *            Callback that will be fired in case there is an error.
     */
    public SubscriptionMessagesHub(String user, String password, boolean clearFilter, NodeErrorListener errorListener) {
        ...
        builder.registerTypeAdapter(LimitOrderCreateOperation.class,
     ...
     * Constructor used to create a subscription message hub that will call the
     * set_subscribe_callback API with the clear_filter parameter set to false,
     * meaning that it will only receive updates from objects we register.
     *
     * @param user
     *            User name, in case the node to which we're going to connect to
     *            requires authentication
        ...
        if (currentId == LOGIN_ID) {
            ...
            if (pendingHandlerList.size() > 0) {
                for (BaseGrapheneHandler handler : pendingHandlerList) {
                    ...
                    mHandlerMap.put(new Long(LOGIN_ID), handler);
                }
            ...
            // If we haven't subscribed to all requested subscription channels
            // yet,
            // just send one more subscription
            if (subscriptionListeners != null && subscriptionListeners.size() > 0
                ...
                payload.add(objects);
                ApiCall subscribe = new ApiCall(databaseApiId, RPC.GET_OBJECTS, payload, RPC.VERSION,
                ...
                if (witnessResponse.result != null && a != null) {
                    // This is the response to a request that was submitted to
                    // the message hub
                    // and whose handler was stored in the "request id" ->
                    // "handler" map
                    ...
                    // If we've already subscribed to all requested subscription
                    // channels, we
                    // just proceed to deserialize content.
                    // The deserialization is handled by all those TypeAdapters
    ...
    private void subscribe() {
        ...
        ApiCall getDatabaseId = new ApiCall(databaseApiId, RPC.CALL_SET_SUBSCRIBE_CALLBACK, subscriptionParams,
                RPC.VERSION, currentId);
        mWebsocket.sendText(getDatabaseId.toJsonString());
    }
    
    /**
     * Public method used to re-establish a subscription after it was cancelled
     * by a previous call to the {@see #cancelSubscriptions()} method call.
     *
     * Please note that you should repeat the registration step for every
     * interested listener, since those were probably lost after the previous
     * {@see #cancelSubscriptions()} method call.
     */
    ...
    /**
     * Method that sends a subscription cancellation request to the full node,
     * and also de-registers all subscription and request listeners.
     */
    public void cancelSubscriptions() {
        ...
        currentId++;
        ApiCall unsubscribe = new ApiCall(databaseApiId, RPC.CALL_CANCEL_ALL_SUBSCRIPTIONS,
                new ArrayList<Serializable>(), RPC.VERSION, currentId);
     ...
     *
     * @return True if the websocket is open and there is an active
     ...
     * 
     * @param handler
     *            The handler of a given request
     ...
     * 
     * @param handler
     *            Handler that should be notified.
     */
    private void dispatchConnectionEvent(BaseGrapheneHandler handler) {
        try {
            ...
            // The underlying websocket was already connected, but from the
            // WebSocketAdapter
            ...
            for (StackTraceElement el : e.getStackTrace()) {
                System.out.println(String.format("at %s.%s(%s:%s)", el.getClassName(), el.getMethodName(),
