...
public class SecureRandomStrengthener {
    ...
    private static final EntropySource mTimeEntropySource = new EntropySource() {
        ...
        final ByteBuffer timeBuffer = ByteBuffer.allocate(Long.SIZE / Byte.SIZE * 2);
    ...
    public static SecureRandomStrengthener getInstance() {
        return new SecureRandomStrengthener(DEFAULT_PSEUDO_RANDOM_NUMBER_GENERATOR);
    }
    ...
    private SecureRandomStrengthener(final String algorithm) {
        if (algorithm == null || algorithm.length() == 0) {
            throw new IllegalArgumentException("Please provide a PRNG algorithm string such as SHA1PRNG");
        }
    ...
    public void addEntropySource(final EntropySource source) {
        if (source == null) {
            throw new IllegalArgumentException("EntropySource should not be null");
        }
    ...
    public void reseed(final SecureRandom secureRandom) {
        ...
        try {
            this.digest.digest(this.seedBuffer.array(), 0, this.seedBuffer.capacity());
