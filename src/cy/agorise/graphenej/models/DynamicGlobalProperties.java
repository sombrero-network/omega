...
/**
 * Class used to deserialize the 'result' field returned by the full node after
 * making a call to the 'get_dynamic_global_properties' RPC.
 */
public class DynamicGlobalProperties extends GrapheneObject implements Serializable {
    ...
    /**
     * Class that will parse the JSON element containing the dynamic global
     * properties object and return an instance of the
     * {@link DynamicGlobalProperties} class.
     */
    public static class DynamicGlobalPropertiesDeserializer implements JsonDeserializer<DynamicGlobalProperties> {
        
        @Override
        public DynamicGlobalProperties deserialize(JsonElement jsonElement, Type typeOfT,
                JsonDeserializationContext context) throws JsonParseException {
            JsonObject jsonObject = jsonElement.getAsJsonObject();
            
            // Creating an instance of the DynamicGlobalProperties
            DynamicGlobalProperties dynamicGlobal = new DynamicGlobalProperties(
                    jsonElement.getAsJsonObject().get(KEY_ID).getAsString());
            ...
            try {
                dynamicGlobal.next_maintenance_time = dateFormat
                        .parse(jsonObject.get(DynamicGlobalProperties.KEY_NEXT_MAINTENANCE_TIME).getAsString());
            } catch (ParseException e) {
                System.out.println("ParseException. Msg: " + e.getMessage());
            }
            ...
            dynamicGlobal.accounts_registered_this_interval = jsonObject
                    .get(DynamicGlobalProperties.KEY_ACCOUNTS_REGISTERED_THIS_INTERVAL).getAsLong();
            dynamicGlobal.recently_missed_count = jsonObject.get(DynamicGlobalProperties.KEY_RECENTLY_MISSED_COUNT)
                    .getAsLong();
            dynamicGlobal.current_aslot = jsonObject.get(DynamicGlobalProperties.KEY_CURRENT_ASLOT).getAsLong();
            dynamicGlobal.recent_slots_filled = jsonObject.get(DynamicGlobalProperties.KEY_RECENT_SLOTS_FILLED)
                    .getAsString();
            dynamicGlobal.dynamic_flags = jsonObject.get(DynamicGlobalProperties.KEY_DYNAMIC_FLAGS).getAsInt();
            dynamicGlobal.last_irreversible_block_num = jsonObject
                    .get(DynamicGlobalProperties.KEY_LAST_IRREVERSIBLE_BLOCK_NUM).getAsLong();
