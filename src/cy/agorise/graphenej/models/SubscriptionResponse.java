...
/**
 * Class that represents a generic subscription response. The template for every
 * subscription response is the following:
 *
 * { "method": "notice" "params": [ SUBSCRIPTION_ID, [[ { "id": "2.1.0", ... },
 * { "id": ... }, { "id": ... }, { "id": ... } ]] ], }
 *
 * As of 1/2017, the witness API returns all sort of events, not just the ones
 * we're interested in once we make a call to the 'set_subscribe_callback',
 * regardless of whether the 'clear_filter' parameter is set to true or false.
 *
 * To minimize CPU usage, we introduce a scheme of selective parsing,
 * implemented by the static inner class SubscriptionResponseDeserializer.
 *
...
public class SubscriptionResponse {
    ...
    /**
     * Inner static class used to parse and deserialize subscription responses
     * in a partial way, depending on the amount of SubscriptionListeners we
     * might have registered.
     *
     * The rationale behind these architecture is to avoid wasting computational
     * resources parsing unneeded objects that might come once the are
     * subscribed to the witness notifications.
     */
    public static class SubscriptionResponseDeserializer implements JsonDeserializer<SubscriptionResponse> {
        /**
         * Map of ObjectType to Integer used to keep track of the current amount
         * of listener per type
         */
         ...
         * a map of ObjectType to integer in order to keep track of how many
         * listeners to each type of object we have.
         */
         ...
         *            Class implementing the {@see SubscriptionListener}
         *            interface to be added to the list.
         */
         ...
         * @param subscriptionListener:
         *            Class implementing the {@see SubscriptionListener}
         *            interface to be removed from the list.
         */
        ...
        @Override
        public SubscriptionResponse deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
            ...
            // Hash map used to record the type of objects present in this
            ...
            for (JsonElement object : subArray) {
                if (object.isJsonObject()) {
                    GrapheneObject grapheneObject = new GrapheneObject(
                            object.getAsJsonObject().get(KEY_ID).getAsString());
                    ...
                    /*
                     * Here's where we apply the selective deserialization
                     * logic, meaning we only completely deserialize an object
                     * contained in a notification if there is at least one
                     * registered listener interested in objects of that type.
                     */
                    if (listenerTypeCount > 0) {
                        ...
                        if (grapheneObject.getObjectType() == ObjectType.ACCOUNT_BALANCE_OBJECT) {
                            ...
                            balanceObject.asset_type = jsonObject.get(AccountBalanceUpdate.KEY_ASSET_TYPE)
                                    .getAsString();
                            ...
                            DynamicGlobalProperties dynamicGlobalProperties = context.deserialize(object,
                                    DynamicGlobalProperties.class);
                            ...
                            BroadcastedTransaction broadcastedTransaction = new BroadcastedTransaction(
                                    grapheneObject.getObjectId());
                            broadcastedTransaction.setTransaction((Transaction) context
                                    .deserialize(jsonObject.get(BroadcastedTransaction.KEY_TRX), Transaction.class));
                            broadcastedTransaction
                                    .setTransactionId(jsonObject.get(BroadcastedTransaction.KEY_TRX_ID).getAsString());
