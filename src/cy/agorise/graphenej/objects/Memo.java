...
public class Memo implements ByteSerializable, JsonSerializable {
    ...
    public static byte[] encryptMessage(ECKey privateKey, PublicKey publicKey, BigInteger nonce, String message) {
        ...
        try {
            ...
            byte[] secret = publicKey.getKey().getPubKeyPoint().multiply(privateKey.getPrivKey()).normalize()
     ...
     * @return: The encrypted version of the message.
     */
    public static byte[] encryptMessage(ECKey privateKey, Address destinationAddress, BigInteger nonce,
            String message) {
     ...
     */
    public static String decryptMessage(ECKey privateKey, PublicKey publicKey, BigInteger nonce, byte[] message)
            throws ChecksumException {
        String plaintext = "";
        try {
            MessageDigest sha256 = MessageDigest.getInstance("SHA-256");
     ...
     *            The private key of the recipient.
     * @param sourceAddress:
     *            The public address key of the sender.
     * @param nonce:
     *            The nonce.
     * @param message:
     *            The encrypted message.
     ...
     * @throws ChecksumException
     */
    public static String decryptMessage(ECKey privateKey, Address sourceAddress, BigInteger nonce, byte[] message)
            throws ChecksumException {
    ...
    public byte[] toBytes() {
        if ((this.from == null) && (this.to == null) && (this.message == null)) {
            ...
            return Bytes.concat(new byte[] { 1 }, new byte[] { (byte) 0 }, new byte[] { (byte) 0 },
                    new byte[] { (byte) 0 }, new byte[] { (byte) this.message.length }, this.message);
        } else {
            
            byte[] paddedNonceBytes = new byte[8];
            byte[] originalNonceBytes = nonce.toByteArray();
            System.arraycopy(originalNonceBytes, 0, paddedNonceBytes, 8 - originalNonceBytes.length,
                    originalNonceBytes.length);
            ...
            return Bytes.concat(new byte[] { 1 }, senderPublicKey.toBytes(), recipientPublicKey.toBytes(), nonceBytes,
                    new byte[] { (byte) this.message.length }, this.message);
        }
    }
     ...
     *
     * This method differs from the {@link #toJson(boolean)} in that here the
     * nonce is encoded as an hexadecimal number, while in that one we offer the
     * user to choose either the decimal or hexadecimal representations.
     *
     ...
     *
     * @param decimal
     *            If true, the nonce is saved as string containing a decimal
    ...
    public static class MemoDeserializer implements JsonDeserializer<Memo> {
        ...
        @Override
        public Memo deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
                throws JsonParseException {
            JsonObject jsonObject = json.getAsJsonObject();
            String fromAddress = jsonObject.get(KEY_FROM).getAsString();
            String toAddress = jsonObject.get(KEY_TO).getAsString();
            
            // Apparently the nonce is always coming from the full node as a
            // string containing a
            // decimal number. This is at odds with the result of the
            // #toJsonObject method
