...
public class CustomOperation extends BaseOperation {
    ...
    public CustomOperation(AssetAmount fee, UserAccount payer, int operationId, List<UserAccount> requiredAuths,
            String data) {
    ...
    public byte[] toBytes() {
        ...
        return Bytes.concat(feeBytes, payerBytes, requiredAuthsLength, requiredAuthsBytes, operationIdBytes, dataLength,
                dataBytes);
    }
     ...
     * custom_operation into its java object version.
     *
     * The following is an example of the serialized form of this operation:
     *
     * [ 35, { "fee": { "amount": 100000, "asset_id": "1.3.0" }, "payer":
     * "1.2.20", "required_auths": [ "1.2.20" ], "id": 61166, "data":
     * "736f6d652064617461" } ]
     */
    public static class CustomOperationDeserializer implements JsonDeserializer<CustomOperation> {
        
        @Override
        public CustomOperation deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
            ...
            if (json.isJsonArray()) {
                ...
                if (serializedCustomOperation.get(0).getAsInt() != OperationType.CUSTOM_OPERATION.ordinal()) {
                    // If the operation type does not correspond to a custom
