 ...
 *
 * The blockchain will atempt to sell amount_to_sell.asset_id for as much
 * min_to_receive.asset_id as possible. The fee will be paid by the seller's
 * account. Market fees will apply as specified by the issuer of both the
 * selling asset and the receiving asset as a percentage of the amount
 * exchanged.
 *
 * If either the selling asset or the receiving asset is white list restricted,
 * the order will only be created if the seller is on the white list of the
...
public class LimitOrderCreateOperation extends BaseOperation {
     ...
     * @param fillOrKill:
     *            If this flag is set the entire order must be filled or the
     *            operation is rejected.
     */
    public LimitOrderCreateOperation(UserAccount seller, AssetAmount toSell, AssetAmount minToReceive, int expiration,
            boolean fillOrKill) {
    ...
    /**
     * Deserializer used to convert the JSON-formatted representation of a
     * limit_order_create_operation into its java object version.
     *
     * The following is an example of the serialized form of this operation:
     *
     * [ 1, { "fee": { "amount": 14676, "asset_id": "1.3.0" }, "seller":
     * "1.2.36449", "amount_to_sell": { "amount": 945472, "asset_id": "1.3.850"
     * }, "min_to_receive": { "amount": 4354658, "asset_id": "1.3.861" },
     * "expiration": "1963-11-25T06:31:44", "fill_or_kill": false, "extensions":
     * [] } ]
     *
     *
     */
    public static class LimitOrderCreateDeserializer implements JsonDeserializer<LimitOrderCreateOperation> {
        ...
        @Override
        public LimitOrderCreateOperation deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
                throws JsonParseException {
            if (json.isJsonArray()) {
                // This block is used just to check if we are in the first step
                ...
                if (serializedTransfer.get(0).getAsInt() != OperationType.LIMIT_ORDER_CREATE_OPERATION.ordinal()) {
                    // If the operation type does not correspond to a transfer
                    // operation, we return null
                    return null;
                } else {
                    // Calling itself recursively, this is only done once, so
                    // there will be no problems.
                    return context.deserialize(serializedTransfer.get(1), LimitOrderCreateOperation.class);
                }
            } else {
                // This block is called in the second recursion and takes care
                // of deserializing the
