...
public class RegisterUsingFaucet {
    ...
    public static String register(String username) {
        ...
        try {
            ...
            // .credentialsForBasic(username, password).credentials(username,
            // password).build();
            // client.register(feature);
            
            // create keys for username and password
            KeysGenerator kg = KeysGenerator.generate(username, RandomPassword.newPassword(70));
            String jsonStr = new AccountJson(kg.account_name, kg.owner_private.publicKey, kg.active_private.publicKey,
                    kg.active_private.publicKey, "", "").json();
            
            // PUT request, if need uncomment it
            // final Response response = client
            // .target("https://localhost:7002/VaquarKhanWeb/employee/api/v1/informations")
            // .request().put(Entity.entity(input, MediaType.APPLICATION_JSON),
            // Response.class);
            // GET Request
            Invocation.Builder reqBuilder = client.target("https://testnet.sombrero.network/api/v1/accounts")
                    .request(MediaType.APPLICATION_JSON_TYPE).accept("application/json")
            ...
            // understands the content type of the
            // request entity (hence a 415(Unsupported Media Type) status code
            // is inappropriate), and the syntax
            // of the request entity is correct (thus a 400 (Bad Request) status
            // code is inappropriate) but
            ...
            // will get 201 when successfull registration, everything else are
            // errors of some sort
                    ...
                    if (obj instanceof JSONObject) {
                        ...
                        if (errorObj != null) {
                            ...
                            if (errorObj.has("name")) {
                                // {"name":["Only lowercase alphanumeric
                                // characters, dashes and periods. Must start
                                // with a letter and cannot end with a dash."]}
                                // most likely mean that regular expression set
                                // to validate username on faucet
                                /// home/c/CLionProjects/faucet/app/models/bts_account.rb
                                // doesn't match to regular expression in
                                // LoginController
