...
public class TransactionTest {
    ...
    /**
     * Generic witness response listener that will just release the lock created
     * in main thread.
     */
    ...
    /**
     * Receives the elements required for building a transaction, puts them
     * together and broadcasts it.
     * 
     * @param privateKey:
     *            The private key used to sign the transaction.
     * @param operationList:
     *            The list of operations to include
     * @param responseListener:
     *            The response listener.
     * @param lockObject:
     *            Optional object to use as a lock
     */
    private void broadcastTransaction(ECKey privateKey, List<BaseOperation> operationList,
    ...
    public void testTransferTransaction() {
        ...
        Memo memo = new Memo(new Address(ECKey.fromPublicOnly(sourcePrivateKey.getPubKey())), new Address(to1.getKey()),
                nonce, encryptedMessage);
        
        // Creating operation 1
        TransferOperation transferOperation1 = new TransferOperationBuilder()
                .setTransferAmount(new AssetAmount(UnsignedLong.valueOf(1), CORE_ASSET)).setSource(bilthon_7)
                .setDestination(bilthon_5) // bilthon-5
                .setFee(new AssetAmount(UnsignedLong.valueOf(FEE_AMOUNT), CORE_ASSET)).setMemo(memo).build();
        
        // Creating operation 2
        TransferOperation transferOperation2 = new TransferOperationBuilder()
                .setTransferAmount(new AssetAmount(UnsignedLong.valueOf(1), CORE_ASSET)).setSource(bilthon_7) // bilthon-15
    ...
    public void testLimitOrderCreateTransaction() {
        ...
        LimitOrderCreateOperation operation = new LimitOrderCreateOperation(seller, amountToSell, minToReceive,
                (int) expiration, false);
    ...
    /**
     * Since tests should be independent of each other, in order to be able to
     * test the cancellation of an existing order we must first proceed to
     * create one. And after creating one, we must also retrieve its id in a
     * separate call.
     *
     * All of this just makes this test a bit more complex, since we have 3
     * clearly defined tasks that require network communication
     *
     * 1- Create order 2- Retrieve order id 3- Send order cancellation tx
     *
        ...
        broadcastTransaction(privateKey, operationList, new WitnessResponseListener() {
            ...
            public void onSuccess(WitnessResponse response) {
                ...
                try {
                    ...
                    mWebSocket.addListener(new GetLimitOrders(base.getObjectId(), quote.getObjectId(), 100,
                                ...
                                public void onSuccess(WitnessResponse response) {
                                    ...
                                    for (LimitOrder order : orders) {
                                        if (order.getSeller().getObjectId().equals(bilthon_7.getObjectId())) {
                                            ...
                                            // Instantiating a private key for
                                            // bilthon-15
                                            ECKey privateKey = new BrainKey(BILTHON_7_BRAIN_KEY, 0).getPrivateKey();
                                            
                                            // Creating limit order cancellation
                                            // operation
                                            LimitOrderCancelOperation operation = new LimitOrderCancelOperation(order,
                                                    bilthon_7);
                                            ArrayList<BaseOperation> operationList = new ArrayList<>();
                                            operationList.add(operation);
                                            
                                            // Broadcasting order cancellation
                                            // tx (Task 3)
                                            broadcastTransaction(privateKey, operationList,
                                                    new WitnessResponseListener() {
