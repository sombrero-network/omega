...
public class GetLimitOrdersTest extends BaseApiTest {
    ...
    public void testGetLimitOrders() {
        try {
            mWebSocket.addListener(
                    new GetLimitOrders(base.getObjectId(), quote.getObjectId(), 100, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
                            ...
                            for (LimitOrder order : orders) {
                                if (order.getSellPrice().base.getAsset().getObjectId().equals(base.getObjectId())) {
                                    ...
                                    baseToQuoteExchange = converter.getConversionRate(order.getSellPrice(),
                                            Converter.BASE_TO_QUOTE);
                                    quoteToBaseExchange = converter.getConversionRate(order.getSellPrice(),
                                            Converter.QUOTE_TO_BASE);
                                    System.out
                                            .println(String.format("> id: %s, base to quote: %.5f, quote to base: %.5f",
                                                    order.getObjectId(), baseToQuoteExchange, quoteToBaseExchange));
                                } else {
    ...
    /**
     * This method is designed to test how the OrderBook class handles data
     * obtained from the GetLimitOrders API call.
     */
    // @Test
            ...
            mWebSocket.addListener(
                    new GetLimitOrders(base.getObjectId(), quote.getObjectId(), 100, new WitnessResponseListener() {
                        ...
                        public void onSuccess(WitnessResponse response) {
                            ...
                            // Testing the successful creation of a limit order
                            // create operation
                            Assert.assertTrue(operation != null);
                            double price = (double) Math.pow(10, base.getPrecision() - quote.getPrecision())
                                    * operation.getMinToReceive().getAmount().longValue()
                                    / operation.getAmountToSell().getAmount().longValue();
