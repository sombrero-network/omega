...
/**
 * compile 'com.neovisionaries:nv-websocket-client:1.30' compile
 * 'org.bitcoinj:bitcoinj-core:0.14.3' compile group: 'com.google.code.gson',
 * name: 'gson', version: '2.8.0' compile group: "org.tukaani", name: "xz",
 * version: "1.6"
 *
...
public class NodeConnectionTest {
     ...
     *
     * Need to setup the NODE_URL_2 env to work. The first hard-coded URL is
     * wrong and will fail. The NodeConnection instance should recover and try
     ...
     *
     * http://docs.bitshares.org/integration/often-used-calls.html#id18
     *
     * Request balances for a valid account (Need to setup the ACCOUNT_ID_1 env
     * with desired account id)
     *
     */
    // @Test
    public void testGetAccountBalancesRequest() {
        ...
        nodeConnection = NodeConnection.getInstance();
        nodeConnection.addNodeUrl(NODE_URL_5);
        // https://wallet.bitshares.org/#/account/constantin-z
        nodeConnection.connect("u-1", "", false, mErrorListener);
        ...
        // nodeConnection.addRequestHandler(new GetAccountBalances(userAccount,
        // assetList, false, new WitnessResponseListener(){
        // @Override
        // public void onSuccess(WitnessResponse response) {
        ...
        // @Override
        // public void onError(BaseResponse.Error error) {
        // System.out.println("getAccountBalances.onError. Msg: "+
        // error.message);
        // }
        // }));
        // }catch(RepeatedRequestIdException e){
        // System.out.println("RepeatedRequestIdException. Msg:
        // "+e.getMessage());
        // }
        ...
        try {
            ...
            nodeConnection
                    .addRequestHandler(new GetAccountBalances(userAccount, null, false, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
                            System.out.println("getAccountBalances.onSuccess");
                        }
     ...
     *
     * Request for a valid account name by name (Need to setup the ACCOUNT_NAME
     * env with desired account name)
     *
     */
    // @Test
     ...
     *
     * Request for a valid account block header (Need to setup the
     * BlOCK_TEST_NUMBER env with desired block height)
     */
    // @Test
    public void testGetBlockHeaderRequest() {
        ...
        try {
            nodeConnection
                    .addRequestHandler(new GetBlockHeader(BlOCK_TEST_NUMBER, false, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
    ...
    public void testGetLimitOrdersRequest() {
        ...
        try {
            nodeConnection.addRequestHandler(
                    new GetLimitOrders(asset_sold_id, asset_purchased_id, limit, true, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
    ...
    public void testGetMarketHistoryRequest() {
        ...
        try {
            nodeConnection.addRequestHandler(new GetMarketHistory(asset_base, asset_quote, bucket, start, end, true,
    ...
    public void testGetRelativeAccountHistoryRequest() {
        ...
        try {
            nodeConnection.addRequestHandler(
                    new GetRelativeAccountHistory(userAccount, stop, limit, start, true, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
    ...
    public void testGetRequiredFeesRequest() {
        ...
        TransferOperation transferOperation = new TransferOperation(userAccount_from, userAccount_to, assetAmountTest,
                feeAmountTest);
        ...
        try {
            nodeConnection
                    .addRequestHandler(new GetRequiredFees(operations, testAsset, true, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
    ...
    public void testGetTradeHistoryRequest() {
        nodeConnection = NodeConnection.getInstance();
        nodeConnection.addNodeUrl(NODE_URL_1);
        nodeConnection.connect("constantin-z", "", false, mErrorListener);
        ...
        // datetime of of the most recent operation to retrieve
        String start = "2018-03-08T22:39:09";
        // datetime of the the earliest operation to retrieve
        // 07/16/2017 1:33pm
        String end = "2018-01-01T22:39:09";
        
        System.out.println("Adding GetTradeHistory request");
        try {
            nodeConnection.addRequestHandler(new GetTradeHistory(asset_sold_id, asset_purchased_id, start, end, limit,
                    true, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
    ...
    /**
     * Test for ListAsset Handler. ' Request the 'list_assets' API call to the
     * witness node
     */
    // @Test
    ...
    public void testGetLookupAccountsRequest() {
        ...
        try {
            nodeConnection
                    .addRequestHandler(new LookupAccounts(userAccount.getName(), true, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
        ...
        try {
            nodeConnection.addRequestHandler(
                    new LookupAccounts(userAccount_2.getName(), maxAccounts, true, new WitnessResponseListener() {
                        @Override
                        public void onSuccess(WitnessResponse response) {
                            System.out.println("LookupAccounts.onSuccess");
                        }
        ...
        ArrayList<Asset> assetList = new ArrayList<Asset>() {
            {
                ...
                // add(BITEUR);
                add(new Asset("1.3.0"));
                add(new Asset("1.3.1570"));
                add(new Asset("1.3.3128"));
                add(new Asset("1.3.3263"));
                add(new Asset("1.3.3265"));
                add(new Asset("1.3.3289"));
                add(new Asset("1.3.3302"));
                add(new Asset("1.3.3341"));
                add(new Asset("1.3.3383"));
            }
        };
    ...
    public void testTransactionBroadcastSequenceRequest() {
        ...
        TransferOperation transferOperation1 = new TransferOperationBuilder()
                .setTransferAmount(new AssetAmount(UnsignedLong.valueOf(1), BTS)).setSource(userAccount)
                .setDestination(userAccount_2).setFee(new AssetAmount(UnsignedLong.valueOf(264174), BTS)).build();
